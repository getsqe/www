...
The first time we ran it, the live test found `iceberg_demo_analytics.iceberg_user_events` (around 1.5 million rows) on account 123456789012 in `eu-example-1`. The second time, we pointed the same code at S3 Tables in `eu-example-1` and got back `testtablebucket/testnamespace/daily_sales`. Both worked. Same code path, different region, different service.
...
By Phase O we had four backend modules in the sqe-catalog crate: `glue.rs`, `hms.rs`, `sql.rs`, and `hadoop.rs`. Each was a thin wrapper around an upstream catalog crate. Each had its own `BackendConfig` struct with the same three or four fields under different names. Each had its own `build_catalog()` method that did the same shape of work: read props, construct an upstream builder, call `.load()`.
...
The loader-refactor MR deleted the wrapper modules. All three of them. Six hundred lines gone in a single commit. the sqe-catalog crate now has a single dispatch site:
...
The matrix evidence column had been pointing at file paths that no longer existed. Every cell that referenced the sqe-catalog crate got its evidence updated to point at the loader dispatch site instead.
