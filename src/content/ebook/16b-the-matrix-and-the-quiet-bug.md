...
I wrote eleven integration tests in the sqe-coordinator crate. Each test stood up a fresh table with a V3-only column type, exercised one matrix cell, and asserted the post-state through the same TVFs (`table_files`, `table_snapshots`) the matrix evidence column already cited.
