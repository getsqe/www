...
Jacob has one. We created an S3 bucket in eu-example-1 to use as the warehouse, dropped the AWS profile name and bucket URI into a gitignored `.env` file, and updated the test to read from environment.
...
S3 Tables exposes itself through the same Iceberg REST protocol that Polaris speaks. The endpoint is `https://<glue-endpoint>/iceberg` (the federated Glue path) or `https://s3tables.<region>.aws.com/iceberg` (per-bucket). Same wire format. Same JSON. Same paths.
...
- Five new test functions in the sqe-catalog crate. Each is `#[ignore]` because they need external services.
...
| `sqe:aws-glue-catalog:v2` | Live AWS round-trip in eu-example-1 |
...
The branch is `a feature branch`. The MR is !113. Score is 158/189 (83.6%).
