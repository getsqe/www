...
Three more sit on the list. There is no `SET TAGS` DDL yet, so associations are authored through `ALTER TABLE ... SET TBLPROPERTIES`, and there is no tag propagation when a column is derived from a tagged one. Policies are not named reusable objects the way Snowflake's `CREATE MASKING POLICY` is; a Ranger policy item is the unit. And the catalog layer has no FUTURE grants, so a `GRANT` covers tables that exist, not tables yet created.
