...
Instead, `MaintenancePrincipal` in the sqe-coordinator crate owns a private `OidcM2mProvider` instance that it constructs directly from `[maintenance.principal]` config. There is no `AuthProviderConfig::M2m` variant. `build_auth_chain` was never touched. The doc comment on the struct says it plainly:
