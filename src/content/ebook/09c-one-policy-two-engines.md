...
The activation is `[policy] engine = "ranger"`, which swaps the PassthroughEnforcer from chapter 9 for `RangerStore` (the sqe-policy crate). The rewriter is the same `PolicyPlanRewriter` the OPA and Cedar backends use.
...
Five session-context UDFs carry the session identity (the sqe-policy crate):
...
Three of three rows, byte-exact, across two engines built by two different communities in two different languages. This was validated live in an earlier change.
