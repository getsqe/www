---
title: "Making SQE Work Everywhere: Pluggable Auth and Catalogs"
description: "How we're turning a single-vendor query engine into something that runs against any identity provider, any catalog, and any cloud."
pubDate: "2026-03-22"
...
Roles feed into the policy engine (when it arrives in Step 5). This decouples "who are you?" from "what can you do?". The auth provider determines identity. The role mapping determines permissions. The policy engine enforces them at query time.

---

## Pluggable catalogs: where is your data?

Auth tells us who the user is. The catalog tells us where the data is. People keep their data in wildly different places.
...
**AWS Glue.** The dominant Iceberg catalog on AWS. Glue exposes a standard Iceberg REST endpoint at `glue.{region}.aws.com/iceberg`, but authenticates with SigV4, not bearer tokens. The Glue backend wraps the Iceberg REST client with AWS IAM signing: instance profile, environment variables, or explicit keys.
...
**On-premise Hadoop migrations** deploy with Hive Metastore + OIDC + Ceph/MinIO. Existing Iceberg tables become queryable immediately.
...
This is planned work, not yet implemented. Pluggable auth is 59 tasks across 7 phases. Pluggable catalogs is 83 tasks across 11 phases. Both are designed, spec'd, and ready to build. They can run in parallel. Auth and catalog are independent trait hierarchies.
