...
The first time we ran it, the live test found `iceberg_demo_analytics.iceberg_user_events` (around 1.5 million rows) on account ACCOUNT_ID in `eu-example-1`. The second time, we pointed the same code at S3 Tables in `eu-example-1` and got back `testtablebucket/testnamespace/daily_sales`. Both worked. Same code path, different region, different service.
